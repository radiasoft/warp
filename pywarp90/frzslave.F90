...
contains
  ...
  SUBROUTINE mpi_send_real_array(r, tid, tag)
    ...
    REAL(8), DIMENSION(:) :: r
!    WRITE(0,*) 'send to ',tid
      call mpi_send(r,SIZE(r),mpi_double_precision,int(tid,4),int(tag,4),MPI_COMM_WORLD,ierr)
!    WRITE(0,*) 'sent to ',tid
...
!    WRITE(0,*) 'send to ',tid
      call mpi_isend(r,SIZE(r),mpi_double_precision,int(tid,4),int(tag,4),MPI_COMM_WORLD,rqu,ierr)
!    WRITE(0,*) 'sent to ',tid
