...
class ElemStatus:
  def isin(self):
    ...
    if self.czs[0] <= top.zbeam and top.zbeam <= self.cze[0]:
      return 1
    return 0
  def update(self):
    ...
    id = self.cid[0]
    isin = self.isin()
    if self.id == id and self.inelem == isin:
      changed = 0
    else:
      changed = 1
    self.id = id
    self.inelem = isin
    return changed
  def __init__(self,czs,cze,ap,ox,oy,cid,sstring,rods):
    self.czs = czs
    self.cze = cze
    self.ap = ap
    self.ox = ox
    ...
    self.elems = eval(self.sstring)
    self.rods = rods*ones(len(self.czs))
    self.id = self.cid[0]
...
class ElementStati:
  ...
  def __init__(self):
    self.stati = []
    if top.quads:
      self.stati.append(ElemStatus(top.cquadzs,top.cquadze,top.quadap,
                                   top.qoffx,top.qoffy,top.cquadid,
                                   'top.quads',greater(abs(top.quadde),0.)))
    if top.heles:
      self.stati.append(ElemStatus(top.chelezs,top.cheleze,top.heleap,
                                   top.heleox,top.heleoy,top.cheleid,
                                 'top.heles',greater(abs(top.heleae[0,:]),0.)))
    if top.emlts:
      self.stati.append(ElemStatus(top.cemltzs,top.cemltze,top.emltap,
                                   top.emltox,top.emltoy,top.cemltid,
                                   'top.emlts',1))
    if top.mmlts:
      self.stati.append(ElemStatus(top.cmmltzs,top.cmmltze,top.mmltap,
                                   top.mmltox,top.mmltoy,top.cmmltid,
                                   'top.mmlts',0))
    if top.pgrds:
      self.stati.append(ElemStatus(top.cpgrdzs,top.cpgrdze,top.pgrdap,
                                   top.pgrdox,top.pgrdoy,top.cpgrdid,
                                   'top.pgrds',1))
    if top.bgrds:
      self.stati.append(ElemStatus(top.cbgrdzs,top.cbgrdze,top.bgrdap,
                                   top.bgrdox,top.bgrdoy,top.cbgrdid,
                                   'top.bgrds',0))
    if top.drfts:
      self.stati.append(ElemStatus(top.cdrftzs,top.cdrftze,top.drftap,
                                   top.drftox,top.drftoy,top.cdrftid,
                                   'top.drfts',0))
