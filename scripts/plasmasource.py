...
plasmasource_version = "$Id: plasmasource.py,v 1.2 2004/10/07 21:10:37 dave Exp $"
...
class PlasmaSource:
  ...
  def go(self,n=1,fix1=0.):
    ...
    for i in range(n):
      gun(ipsave=100000,maxtime=1.,ipstep=3)
      self.rhoi = frz.basegrid.rho[0:self.nx+1,0:self.nz+1] + 0.
      frz.basegrid.rho[:,:] = where(
                 frz.basegrid.phi[1:-1,1:-1]>self.sourcevolt+fix1,
                                    0.,frz.basegrid.rho)
      fieldsol(-1)
